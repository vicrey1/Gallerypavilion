...
export async function GET(request: NextRequest) {
  ...
  const recentPhotographers = await (prisma as any).photographer.findMany({
      take: 5,
    ...
    // Recent galleries
  const recentGalleries = await (prisma as any).gallery.findMany({
      take: 5,
      orderBy: { createdAt: 'desc' },
      include: {
        photographer: {
    ...
    // Recent clients
  const recentClients = await (prisma as any).client.findMany({
      take: 5,
      orderBy: { createdAt: 'desc' },
      include: {
        user: {
  ...
  const recentPurchases = await (prisma as any).purchaseRequest.findMany({
      take: 5,
      orderBy: { createdAt: 'desc' },
      include: {
        client: {
